...
import org.jeecg.modules.flowable.domain.dto.*;
...
public class FlowTaskServiceImpl extends FlowServiceFactory implements IFlowTaskService {
    ...
    public Result complete(FlowTaskVo taskVo) {
        ...
        if (DelegationState.PENDING.equals(task.getDelegationState())) {
            ...
            log.info("complete  current task:"+ task.getName()+"   taskId:"+task.getId()+"    processInstanceId: "+task.getProcessInstanceId()+"   getCategory: "+task.getCategory());
        ...
        prepareNextFlowNode(task,business,taskVo,nextFlowNode);
...
//        }
    ...
    public void prepareNextFlowNode(Task task,
        ...
        if(nextFlowNode!=null){
            log.info("prepareNextFlowNode method  nextFlowNode: "+ nextFlowNode.getUserTask().getName()+"   taskId:"+nextFlowNode.getUserTask().getId());

        }
        List<Task> task2List = taskService.createTaskQuery().processInstanceId(business.getProcessInstanceId()).active().list();
        Task nextTask =null;
        //节点较正
        if (task2List!=null&&task2List.size()>0&&
                nextFlowNode!=null&&
                nextFlowNode.getUserTask()!=null){
//          确保下一个节(点通过两种方式取到的一致)
            for (Task task2 : task2List) {
                if(task2.getTaskDefinitionKey().equals(nextFlowNode.getUserTask().getId())){
                    nextTask=task2;
                    log.info("--->find next task by compare:"+nextTask.getName()+"   taskDefinitionKey:"+nextTask.getId());
                }
            }
        }
        //默认选一个可用的下一个节点
        if (CollUtil.isNotEmpty(task2List)&&
            ...
            log.info("--->get next task by random:"+nextTask.getName()+"   taskDefinitionKey:"+nextTask.getTaskDefinitionKey());
        ...
        if(nextTask!=null&&taskVo!=null){
            beforeParamsCandidateUsernames = flowCallBackService.flowCandidateUsernamesOfTask(nextTask,taskVo.getValues());
        }
        List<String> nextTaskCandidateUsers = beforeParamsCandidateUsernames;
        //将当前用户作为下一任务的节点处理候选人(不合理)
...
//        }
        ...
        if (nextTask!=null && task.getTaskDefinitionKey().equals(nextTask.getTaskDefinitionKey())){
            //    * 当前节点是会签节点，没有走完
            log.info("**** 与会签有关？  nextTask: "+ nextTask.getName()+"   task.definitionKey:"+task.getTaskDefinitionKey());
...
//            }
            ...
            if (nextFlowNode!=null) {
                //**有下一个节点
                nextUserTask = nextFlowNode.getUserTask();
...
//                collect_username = nextFlowNodeUserList.stream().map(SysUser::getUsername).collect(Collectors.toList());
                ...
                if (CollUtil.isNotEmpty(nextTaskCandidateUsers)){
                    //    前端传入候选人
                    collect_username = nextTaskCandidateUsers;
                }
            }
            else if(nextTask!=null ){
                nextUserTask =new UserTask();
                nextUserTask.setId(nextTask.getTaskDefinitionKey());
                nextUserTask.setName(nextTask.getName());
                //这里指定的处理候选人即当节点处理人
//                collect_username=taskVo.getCandidateUsers();

                //设置一下一节点处理候选人
                collect_username=nextTaskCandidateUsers;
//                nextUserTask.setPriority(nextTask.getPriority());
            }
            ...
            if(nextUserTask!=null){
                ...
                JSONArray todosArr=JSONArray.parseArray(JSON.toJSONString(collect_username));
                //*** 这里用方法代替
                updateNextNodeCadidate(
                        business,
                        task2List,
    ...
    public List<FlowCurrentUserAndTaskBusi> getCurrentUserBusiness() {
        SysUser loginUser = iFlowThirdService.getLoginUser();
        log.info("current loginUser---> "+loginUser.getUsername());

        List<FlowCurrentUserAndTaskBusi> currentUserBusinessList=new ArrayList<>();
//        ProcessDefinition processDefinition = repositoryService.createProcessDefinitionQuery().processDefinitionId(task.getProcessDefinitionId()).singleResult();
        List<Task> myTaskList= taskService.createTaskQuery().taskCandidateOrAssigned(loginUser.getUsername()).active().list();
        if(myTaskList!=null&&myTaskList.size()>0){
            myTaskList.forEach((task) -> {
                FlowCurrentUserAndTaskBusi userAndTask=new FlowCurrentUserAndTaskBusi();
                FlowMyBusiness business= flowMyBusinessService.getByProcessInstanceId(task.getProcessInstanceId());
                if(business!=null) {
                    log.info("my task--->" + task.getName() + "    task id:" + task.getId() + "    process-instance id:" + task.getProcessInstanceId() + "   business dataId:" + business.getDataId());
                    userAndTask.setTask(task);
                    userAndTask.setCurrentUser(loginUser);
                    userAndTask.setBusiness(business);
                    currentUserBusinessList.add(userAndTask);
                }
            });
        ...
        return currentUserBusinessList;

    }

    private void updateNextNodeCadidate(FlowMyBusiness business,
                                        List<Task> task2List,
                                        JSONArray todosArr){
//        List<String> candidateUsers=taskVo.getCandidateUsers();
//        List<String> candidateUsers=todosArr.toJavaList("String");
    ...
    public Result completeByDateId(FlowTaskVo flowTaskVo){
        ...
        if(dataId==null || dataId.trim().equals("")){
            log.info("***dataId is null");
            return Result.error("***dataId is null");

        }
//        FlowMyBusiness business = flowMyBusinessService.getByDataId(dataId);
        List<FlowCurrentUserAndTaskBusi> myBusiList= getCurrentUserBusiness();
        if(myBusiList!=null&&myBusiList.size()>1){

        }
        for(FlowCurrentUserAndTaskBusi busi: myBusiList) {
            if(busi.getBusiness()!=null){
               if(busi.getBusiness().getDataId().equals(dataId)){
                   flowTaskVo.setTaskId(busi.getTask().getId());
                   flowTaskVo.setInstanceId(busi.getBusiness().getProcessInstanceId());
                   log.info("***math that task:"+busi.getTask().getName()+"   dataid:"+dataId+"    user:"+busi.getCurrentUser().getUsername());
                  break;
               }
            }
...
//        flowTaskVo.setInstanceId(business.getProcessInstanceId());
    ...
    public void taskRejectByDataId(FlowTaskVo flowTaskVo){
        ...
        List<FlowCurrentUserAndTaskBusi> myBusiList= getCurrentUserBusiness();
        if(myBusiList!=null&&myBusiList.size()>0){
            for(FlowCurrentUserAndTaskBusi busi: myBusiList) {
                if(busi.getBusiness()!=null&&busi.getBusiness().getDataId().equals(business.getDataId())){
                    flowTaskVo.setTaskId(busi.getTask().getId());
                    break;
                }
    ...
    public void taskReject(FlowTaskVo flowTaskVo) {
        ...
        try {
            ...
            List<String> beforeParamsCandidateUsernames = flowCallBackService.flowCandidateUsernamesOfTask(task2, values);
            //设置数据
            String doneUsers = business.getDoneUsers();
            // 处理过流程的人
            JSONArray doneUserList = new JSONArray();
...
//                    List<SysUser> sysUserFromTask = getSysUserFromUserTask(targetTask);
                    List<SysUser> sysUserFromTask = getSysUserFromTask(task2);
        ...
        try {
            ...
            if (target!=null){
                ...
                //    开始节点。设置处理人为申请人
                    ...
                    List<SysUser> sysUserFromTask = getSysUserFromUserTask(target2);
                    List<String> collect_username = sysUserFromTask.stream().map(SysUser::getUsername).collect(Collectors.toList());
                    List<String> candidateUsers = flowTaskVo.getCandidateUsers();
                    if (CollUtil.isNotEmpty(candidateUsers)){
                        collect_username = candidateUsers;
                    ...
                    List<String> beforeParamsCandidateUsernames = flowCallBackService.flowCandidateUsernamesOfTask(targetTask, values);
                    if (CollUtil.isNotEmpty(beforeParamsCandidateUsernames)){
                        if (CollUtil.isNotEmpty(candidateUsers)){
                            beforeParamsCandidateUsernames = candidateUsers;
                        }
...
//                    log.info("--->history  task:"+JSON.toJSONString(histIns));
                    FlowTaskDto flowTask = new FlowTaskDto();
...
//                return nextDtos;
    ...
    public List<SysUser> getSysUserFromUserTask(UserTask userTask) {


       if(userTask==null){
           log.info("**getSysUserFromUserTask input userTask is null!--->");
           return Lists.newArrayList();

       }
        String assignee = userTask.getAssignee();//userTask.getAssignee();
        if (StrUtil.isNotBlank(assignee)){
            // 指定单人
            SysUser userByUsername = iFlowThirdService.getUserByUsername(assignee);
            if(userByUsername!=null) {
                log.info("**getSysUserFromUserTask get user--->" + userByUsername.getUsername() + "   by task:" + userTask.getName());
    ...
    public List<SysUser> getSysUserFromTask(Task task) {
...
//        List<String> candidateUsers = task.getc();
//        if (CollUtil.isNotEmpty(candidateUsers)){
//            // 指定多人
//            List<SysUser> list = iFlowThirdService.getAllUser();
//            return list.stream().filter(o->candidateUsers.contains(o.getUsername())).collect(Collectors.toList());
